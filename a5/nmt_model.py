...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.encoder = nn.LSTM(input_size=word_embed_size, hidden_size=hidden_size, bidirectional=True, bias=True)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        lstm_hidden_encodings, decoder_init_state = self.encode(source_padded_chars, source_lengths)
        sentence_masks = self.generate_sent_masks(lstm_hidden_encodings, source_lengths)
        combined_outputs = self.decode(lstm_hidden_encodings, sentence_masks, decoder_init_state, target_padded_chars)
        ### END YOUR CODE
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        init_decoder_cell = self.c_projection.forward(last_cell)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70) -> List[
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            y_t_embed = self.model_embeddings_target.forward(y_tm1)
