...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            self.stack.append(self.buffer.pop(0))  # inefficient but ok..
        elif transition == "LA":
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        # good speedup if we just look at and delete from tail of the list
        batch = unfinished_parses[-batch_size:]
        predicted = model.predict(batch)
        for partial_parse, transition in zip(batch, predicted):
            partial_parse.parse_step(transition)
        i, c = 1, 0
        while c < batch_size and i < len(unfinished_parses) + 1:
