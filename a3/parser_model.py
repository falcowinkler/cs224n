...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.init.xavier_uniform(
            nn.Parameter(torch.zeros(self.embed_size * self.n_features, self.hidden_size)))
        self.embed_to_hidden_bias = nn.init.uniform(nn.Parameter(torch.zeros(self.hidden_size)))
        self.dropout = nn.Dropout(self.dropout_prob)
        self.hidden_to_logits_weight = nn.init.xavier_uniform(nn.Parameter(torch.zeros(hidden_size, n_classes)))
        self.hidden_to_logits_bias = nn.init.uniform(nn.Parameter(torch.zeros(n_classes)))
        ### END YOUR CODE
